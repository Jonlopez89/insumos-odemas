...
- **Carry-over creciente de pedidos tardíos en alguna tienda** (tienda que aparece como "sin pedido / carry-over" en > 20% de las semanas): señal de que el jefe está sistemáticamente atrasándose y la tienda termina servida de forma inconsistente. Detectable vía el surfacing pasivo de FR-061/FR-066 (D-13).
...
- **FR-016** *(reformulado, decisión OQ-124 cerrada 2026-05-26b)* — Ingestar **ventas con granularidad semanal** provistas como **reporte/archivo aportado por el owner** (Jonathan), siguiendo el patrón de ingesta de `docs/` (CSV/XLSX, descubrimiento por patrón de nombre, encoding y parsing dual de fechas per `project-context.md`). Hoy el histórico disponible es **mensual**; el motor requiere cadencia semanal (cada semana hay tiendas en ciclo), así que el owner entrega el corte semanal (por semana × tienda × SKU venta) como contexto del ciclo. **Sin integración a Big Data/BigQuery en v1** (decisión 2026-05-26b: ventas semanales = reporte del owner, mismo patrón que ALLOC/TFS — elimina la dependencia BigQuery de v1). Política fail-loud ante formato inválido o ausencia del archivo. Formato y cadencia del reporte (alineados al ritmo semanal lunes) a definir; fallback de prorrateo mensual→semanal solo si un ciclo no tiene corte semanal disponible, con la advertencia de error asociada (R-12).
- **FR-017** *(reformulado, decisión OQ-125 cerrada 2026-05-26b)* — Ingestar **ALLOC y TFS** (asignaciones y transferencias: lo pedido, lo en curso, lo que va a llegar, lo entregado) como input del cálculo del pedido (FR-040) y del dashboard de validación pedido-vs-recibido (FR-067). Fuente: **reporte/archivo aportado por el owner** (Jonathan) como contexto del ciclo, siguiendo el patrón de ingesta de `docs/` (convive con la ingesta por patrón `ALLOC_*.csv` / `TFS_*.csv` de FR-010). **Sin integración a RMS/BigQuery en v1** (decisión 2026-05-26b: el owner aporta el reporte; los coordinadores no acceden RMS y no se requiere extract automatizado). Política fail-loud ante formato inválido. Formato y cadencia del reporte a definir.
...
- **FR-030** — Calcular **demanda esperada de venta semanalmente** para el próximo ciclo de cada tienda en ciclo activo esa semana, por tienda × SKU venta, usando modelo estacional configurable (familia de modelos en Java puro definida en `project-context.md`). La unidad temporal interna del modelo y el output al coordinador es el ciclo quincenal de la tienda. **Criterio de aceptación (done):** para una tienda con ≥ 12 meses de histórico de ventas, el motor produce un valor de demanda esperada por tienda × SKU venta para el ciclo objetivo, acompañado del trazo de FR-034; el backtesting (`BacktestingSuite.java`) sobre histórico real produce la desviación vs ventas YoY usada como criterio de éxito (R-7/C-O40). **Tiendas con < 12 meses de histórico (cold-start):** el tratamiento (histórico distrital agregado / bootstrap con tienda similar / esperar 12 meses) queda abierto en **OQ-108** (dueño: arquitectura); hasta resolverlo, una tienda sin histórico suficiente se marca explícitamente como **"sin sugerencia del motor"** en la bandeja (fail-loud — no se inventa un número, consistente con `project-context.md`) y el coordinador la trata manualmente.
...
- **FR-035** *(reformulado por D-9, 2026-05-26 — revierte el modelo dual D-4)* — Aplicar **un único `factor_perdida`** (porcentaje de la demanda de venta) resuelto por jerarquía `factor_por_tipo_papel → default_global (≈0.9%)` (configurado en C-9 vía FR-082). El motor lo consume como ajuste porcentual sobre la demanda de venta predicha. **Criterio de aceptación (done):** para una tienda con demanda de venta esperada `V` (FR-030) y factor resuelto `f`, la demanda ajustada cumple `demanda_ajustada = V × (1 + f)` **sobre la demanda de venta, antes de la conversión a insumo (FR-031) y la cuantización (FR-032)**; el trazo explicable (FR-034) declara `V`, `f`, el nivel del que provino `f` (factor por tipo de papel o default global) y el resultado. **La opción técnica interna** de cómo se implementa el ajuste (multiplicador sobre el output del modelo, ajuste del histórico, etc. — F-05) es decisión de `architecture.md`; el criterio de aceptación anterior es observable e independiente del método interno.
...
- **FR-066** *(reformulado, decisión D-13 2026-05-26b — revierte parcialmente D-6)* — **Surfacing pasivo de pedidos tardíos (sin regla de arrastre).** Si una tienda en ciclo nominal de la semana N no envió solicitud antes del corte del miércoles, el sistema **la muestra como `carry-over` / `sin pedido esta semana` en la bandeja** (estado visible pasivo e informativo, coherente con D-7 y FR-061) para que el coordinador tenga visibilidad. **v1 NO modela el carry-over como regla del sistema:** no reinscribe automáticamente la tienda a otro ciclo, **no recalcula ni traslada presupuesto**, y no ejecuta una mecánica de arrastre con bitácora propia. El faltante de una tienda que no pidió **se autocorrige de forma natural** en el siguiente ciclo, porque el forecast es sales-driven (~99% ventas) y la posición de inventario (ALLOC/TFS, FR-017) refleja lo pendiente — *"no se registra, se hace todo operativo y en las ventas e inventario sale de manera natural"* (owner 2026-05-26b). El techo presupuestal sigue siendo **semanal y rígido por tienda** (sin acumulación ni pool — consistente con §2 / OQ-2). La medición o gestión activa del arrastre, si alguna vez se requiere, es trabajo futuro fuera de v1. *(Esta reducción cierra el finding high del Validate 2026-05-26 sobre el paso 4: ya no existe una regla de presupuesto en disputa.)*
...
- **Ventas semanales — reporte aportado por el owner** *(decisión OQ-124 cerrada 2026-05-26b)* — el owner (Jonathan) entrega el corte de ventas semanales (por semana × tienda × SKU venta) como archivo/reporte que el sistema ingesta (FR-016). **Sin integración a Big Data/BigQuery en v1.**
- **ALLOC y TFS — reporte aportado por el owner** *(decisión OQ-125 cerrada 2026-05-26b)* — el owner entrega ALLOC/TFS (lo pedido, en curso, por llegar, entregado) como archivo/reporte que alimenta FR-017/FR-040/FR-067. **Sin integración a RMS/BigQuery en v1**; los coordinadores no acceden RMS y no se requiere extract automatizado.
- *(Nota 2026-05-26b: al aportar ventas semanales y ALLOC/TFS como reportes del owner, v1 **deja de depender de BigQuery** — desaparece el roce "BigQuery-lectura vs BigQuery ML" frente a `project-context.md` para estos inputs.)*
...
- **Mitigación triple:**
  1. **SLA de frescura de snapshot SIM** (NFR-DAT-4 = 36h máximo de antigüedad, endurecido 2026-05-25 — 24h ciclo + 12h buffer). Si se excede, el sistema rechaza operar y notifica al administrador.
...
### R-12 — Dependencia de reportes de ventas semanales y ALLOC/TFS aportados por el owner *(severidad: baja — reencuadrada 2026-05-26b)*

- **Descripción:** la cadencia semanal (D-8) y el cálculo del pedido con ALLOC/TFS (D-10) dependen de datos que, por decisión 2026-05-26b, **aporta el owner (Jonathan) como reportes/archivos** (OQ-124 y OQ-125 cerradas), no de una integración a RMS/BigQuery. Esto **elimina la dependencia de sistemas externos no controlados** y a Fernando como ruta crítica. **Riesgo residual:** depende de que el owner produzca los reportes cada ciclo con formato consistente — punto único de dependencia operativa. Hoy las ventas solo están a nivel mensual; el owner debe poder generar el corte semanal.
- **Mitigación:** fijar **formato y cadencia** de los reportes (alineados al ritmo lunes) y validación fail-loud en ingesta (FR-016, FR-017); documentar el procedimiento del owner para generar los extracts; fallback de prorrateo mensual→semanal solo si un ciclo no tiene corte semanal disponible (con la advertencia de error).
- **Dueño tentativo:** PM + owner (Jonathan).
...
**Decisión (2026-05-25, D-6):** v1 modela el carry-over automáticamente vía FR-066 — salto a la siguiente semana con bitácora, ajuste de presupuesto y surfacing visual.

**Revisión 2026-05-26b (D-13 — revierte parcialmente D-6):** el owner retiró la parte activa. v1 **no** modela el carry-over como regla (sin reinscripción automática, sin recálculo/traslado de presupuesto, sin mecánica de arrastre); se conserva **solo el surfacing pasivo e informativo** en la bandeja (coherente con D-7). El faltante se autocorrige vía el forecast de ventas + posición de inventario en el siguiente ciclo (*"se hace todo operativo y en las ventas e inventario sale de manera natural"*). Ver FR-066 reformulado y FR-061 estado "carry-over de semana anterior".
...
### OQ-124 — Fuente y disponibilidad de ventas semanales — **CERRADA (2026-05-26b)**

**Decisión del owner:** las ventas semanales las **aporta el owner (Jonathan) como reporte/archivo** (mismo patrón que ALLOC/TFS), no vía Big Data/BigQuery. FR-016 reformulado. Elimina la dependencia BigQuery de v1 y el roce con `project-context.md`; Fernando deja de ser ruta crítica. Riesgo residual (producir el corte semanal cada ciclo) trasladado a R-12. Formato y cadencia del reporte a definir (alineados al ritmo lunes); fallback de prorrateo mensual→semanal solo si un ciclo carece de corte semanal.

### OQ-125 — Fuente y extracción de ALLOC/TFS — **CERRADA (2026-05-26b)**

**Decisión del owner (C-O42):** ALLOC/TFS los **aporta el owner como reporte/archivo** que alimenta FR-017/FR-040/FR-067, no vía integración RMS/BigQuery. FR-017 reformulado, §10.6 actualizado, R-12 baja a severidad baja. Los coords no acceden RMS y no se requiere extract automatizado. Formato y cadencia del reporte a definir.

### OQ-126 — Layout del reporte de necesidad de demanda — **CERRADA (2026-05-26b, decisión D-11 = Opción A)**

**Decisión del owner (C-O43):** **Opción A — por SKU → tiendas + cantidades.** La vista de trabajo del coordinador se organiza por SKU. El export al comprador (FR-110) mantiene la estructura del archivo de referencia; el dashboard de validación (FR-067) conserva su vista `proveedor → tienda → SKU`. FR-110 actualizado.
...
- **Coordinador (de distrito):** uno de tres roles (Carlos, Marco, Eduardo) que trabaja su distrito asignado territorialmente **todas las semanas** — tanto consolidación regular como excepciones (asignación **fija, no rotativa**). La "guardia rotativa" aplica solo a tareas inter-distritales que organizan fuera del sistema.
...
- **Carry-over (actualizado 2026-05-26b):** estado **visible pasivo e informativo** de una tienda que no envió solicitud antes del corte del miércoles en su ciclo nominal. v1 **no lo modela como regla** (no reinscribe ni recalcula presupuesto — decisión D-13, revierte parcialmente D-6); el faltante se autocorrige vía el forecast de ventas + posición de inventario en el siguiente ciclo. Surface visual en FR-061 / FR-066 reformulado.
...
**Estado actual del PRD:** `draft` — actualizado el **2026-05-26** (cuarta ronda, *2026-05-26b*) conforme a:
- `.change-signal-2026-05-26b.md` (cuarta ronda — **cierre de hallazgos**: carry-over reducido a surfacing pasivo (D-13, revierte parcial D-6); ventas semanales y ALLOC/TFS como **reporte aportado por el owner** (OQ-124/125 cerradas, sin BigQuery/RMS en v1); layout del reporte = **Opción A** (OQ-126 cerrada, D-11); criterios de aceptación añadidos a FR-035 y FR-030; drifts limpiados. Reviewer Gate: Grade Fair→Good).
- `.change-signal-2026-05-26.md` (tercera ronda — **reversa del modelo dual a factor único de pérdida por tipo de papel**, encuadre de 3 fases, forecast 99% ventas, ALLOC/TFS como input del pedido, FR-061 resuelto, dashboard de validación, inventario no forzado).
...
**Decisiones firmadas en update 2026-05-26:** **D-9** (factor único de pérdida por tipo de papel — revierte D-4), **D-8** (ventas semanales — fuente luego reencuadrada a reporte del owner en 2026-05-26b), **D-12** (v1 = reporte + bandeja con coord; solicitud automática = futuro), **D-11** (layout del reporte — luego decidido Opción A en 2026-05-26b). FR-061 resuelto (coord en pantalla, Excel es output). OQ-106 y OQ-120 cerradas N/A. OQ-123 abierta; OQ-124/125/126 cerradas en 2026-05-26b. R-1, R-3, R-7 severidad reducida; R-12 nuevo (luego bajado a severidad baja). *(FR-066 paso 4, que aquí quedaba pendiente, se resolvió en 2026-05-26b — D-13, surfacing pasivo sin regla de presupuesto.)*

**Findings del Validate resueltos:** FR-061 (high) ✅, sub-riesgo R-1 sin gate (high) ✅ disuelto, OQ-120/OQ-106 (medium) ✅ cerradas N/A. **FR-066 carry-over (high):** ✅ cerrado en 2026-05-26b (D-13, surfacing pasivo). **FR-035/FR-034 y FR-030 (high de redacción):** ✅ cerrados en 2026-05-26b (criterios de aceptación observables). Findings medium/low remanentes (UJ del comprador, SolicitusDeInsumosTodos.xlsx en project-context.md, FR-110 bit-a-bit) → diferidos.
...
**Reviewer Gate — estado tras update 2026-05-26b (Grade Good, 0C/0H/4M/3L):** los 3 High (carry-over + 2 de redacción) cerrados. R-7 reencuadrado (criterio = "sin desviaciones vs YoY", deja de ser bloqueo de tesis). R-8 severidad baja (algoritmo simplificado). R-9 baseline de tiempo cerrado (9h); faltan baselines de quiebre/sobre-inventario + backtesting. **Bloqueos restantes = precondiciones de PILOTAJE, no de aprobación del documento:** spike Java (R-8), backtesting+baselines (R-9), sesión Finanzas (R-7, opcional). R-12 bajado a severidad baja (ventas/ALLOC/TFS = reporte del owner, sin RMS/BigQuery).

**Decisiones pendientes restantes (no bloqueantes del documento):** D-5 (lista de eventos comerciales con Operaciones), OQ-123 (umbral de detección residual), OQ-108 (cold-start de tiendas < 12 meses — arquitectura), y definir el **formato/cadencia de los reportes del owner** (ventas semanales y ALLOC/TFS). *(Cerradas en 2026-05-26b: FR-066 carry-over (D-13), OQ-124, OQ-125, OQ-126 (D-11). La distinción BigQuery-vs-ML quedó moot para estos inputs al no depender de BigQuery.)*
